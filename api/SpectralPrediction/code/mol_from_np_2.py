...
def xyz2mol(atomicNumList, charge, xyz_coordinates, charged_fragments, quick):
    ...
    AC, mol = xyz2AC(atomicNumList, xyz_coordinates,0)
    # xyz2AC : 获得邻接矩阵和mol分子
      
    # Convert AC to bond order matrix and add connectivity and charge info to mol object
    new_mols = AC2mol(mol, AC, atomicNumList, charge, charged_fragments, quick)
...
# ------------------------------------------------------
def matrix_from_xyz(atomicNumList,  xyz_coordinates):
    AC, mol = xyz2AC(atomicNumList, xyz_coordinates,0)
    matrix_xyz = np.zeros(shape=[2,1],dtype=np.int16)
    matrix_xyz_dot = dok_matrix((AC.shape[0],AC.shape[0]),dtype= np.int64)
    for i in range(AC.shape[0]):
        for j in range(AC.shape[1]):
            if AC[i,j] == 1:
                matrix_xyz_dot[i,j] = 1 
                matrix_xyz_temp = np.array([[i],[j]],dtype=np.int16)
                matrix_xyz = np.c_[matrix_xyz,matrix_xyz_temp]


    matrix_xyz = matrix_xyz[:,1:]
    return matrix_xyz,matrix_xyz_dot
  # ------------------------------------------------------  
